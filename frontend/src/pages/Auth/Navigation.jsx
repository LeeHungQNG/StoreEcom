    ...
    >
      <div className="flex flex-col justify-center space-y-4">
        ...
        <Link to="/cart" className="flex relative">
          ...
          <div className="absolute top-9">
            {cartItems.length > 0 && (
              <span>
                <span className="px-1 py-0 text-sm text-white bg-pink-500 rounded-full">{cartItems.reduce((a, c) => +a + +c.qty, 0)}</span>
              </span>
            )}
          </div>
        </Link>
      ...
      <div className="relative">
        ...
        {dropdownOpen && userInfo && (
          <ul className={`absolute right-0 mt-2 mr-14 space-y-2 bg-back text-gray-600 ${!userInfo.isAdmin ? '-top-20' : '-top-80'} `}>
            {userInfo.isAdmin && (
              <>
                <li>
